...
public class SheepController : MonoBehaviour
{
    ...
    {
        health -= damage;
        AudioSorce.clip = bite;
        if(!AudioSorce.isPlaying) AudioSorce.Play();
    }
    ...
    void Update()
    {
        ...
        if (!AudioSorce.isPlaying && timer_sheep <= 0 && Vector2.Distance(GameObject.FindGameObjectWithTag("Player").transform.position, transform.position) < 15)
        {
            int random_sound = Random.Range(0, sheep_sound.Length);
            AudioSorce.clip = sheep_sound[random_sound];
            AudioSorce.Play();
            timer_sheep = sheep_timer_max;
